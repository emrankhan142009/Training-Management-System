    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var checkroleid = _context.Roles.Where(r => r.Id == roleid).FirstOrDefault();
                var roletype = checkroleid.Name;
                   ...
                   if (roletype == "Coordinator")
                    {
                        ...
                        return RedirectToAction("LandingPage", "Coordinators");
                    }

                   else if (roletype == "Instructor")
                    {
                        var getIid = _context.Instructor.Where(u => u.Email == model.Email).FirstOrDefault();
                        //TempData["Instructorid"] = Iid.Id;
                        int Iid = getIid.Id;
                        HttpContext.Session.SetInt32("InstructorId", Iid);

                        _logger.LogInformation("User logged in.");
                        return RedirectToAction("LandingPage", "Instructors");
                    }

                    else if(roletype == "Trainee")
                    {
                        var getTid = _context.Trainee.Where(u => u.Email == model.Email).FirstOrDefault();
                        //TempData["Traineeid"] = Tid.Id;
                        int Tid = getTid.Id;
                        HttpContext.Session.SetInt32("TraineeId", Tid);

                        _logger.LogInformation("User logged in.");
                        return RedirectToAction("LandingPage", "Trainees");
                    }
                    else
                    {
                        return Content("User Type is Unknown");
                    }
